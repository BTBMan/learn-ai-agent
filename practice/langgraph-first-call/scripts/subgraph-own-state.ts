...
const ReviewState = new StateSchema({
  ...
  issues: new ReducedValue(z.array(z.string()).default([]), {
...
const PublishState = new StateSchema({
  ...
  draft: z.string().default(''),
  reviewIssues: new ReducedValue(z.array(z.string()).default([]), {
    reducer: (current, update) => [...current, ...update],
  }),
...
const runReviewPipeline: GraphNode<typeof PublishState> = async (
  state,
  config,
) => {
  ...
  return {
    ...
    reviewPassed: reviewResult.passed && !reviewResult.issues.length,
  };
};
...
const result = await graph.invoke({ topic: '子图如何复用审核流程' });
